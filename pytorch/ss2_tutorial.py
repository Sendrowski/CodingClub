...
print(list(SST2(split="train"))[:10])
print(f"Training set size: {len(list(train_datapipe))}")
...
for sample in SST2(split="dev"):
    tokens, label = apply_transform(sample)
    input = F.to_tensor([tokens], padding_value=padding_idx).to(DEVICE)
    output = model(input)
    prediction = output.argmax(1).item()

    if prediction != label:
        print(f"Prediction = {prediction}, Ground Truth = {label}", f"text = {sample[0]}")
